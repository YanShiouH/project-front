  ...
  <v-container>
    <v-row>
        ...
        <v-table>
          ...
          <tbody>
            <tr v-for="item in cart" :key="item._id" :class="{ 'bg-red-lighten-5': !item.product.sell }">
              <td><v-img :src="item.product.image" cover width="200" aspect-ratio="1"></v-img></td>
              <td>{{ item.product.name }}</td>
              <td>{{ item.product.price }}</td>
              <td>
                <v-btn color="primary" variant="icon" icon="mdi-minus" @click="updateCart(item.product._id, -1)"></v-btn>
                {{ item.quantity }}
                <v-btn color="primary" variant="icon" icon="mdi-plus" @click="updateCart(item.product._id, 1)"></v-btn>
              </td>
              <td>{{ item.quantity * item.product.price }}</td>
              <td><v-btn color="red" variant="icon" icon="mdi-delete"
                  @click="updateCart(item.product._id, item.quantity * -1)"></v-btn></td>
            </tr>
      ...
      <v-col cols="12" class="text-center">
        <p>總金額:{{ total }}</p>
        <v-btn color="green" @click="submit" :disabled="!canCheckout">結帳</v-btn>
      </v-col>
    </v-row>
  </v-container>
</template>
